...
+ 对象的类型决定了对其可执行的操作，C++在编译时会进行类型检查，是一种静态语言，所以变量必须先声明后使用。分开定义和声明，使得C++支持分开编译，一个文件定义，多个文件声明。
  ...
  + 声明declaration：确定类型和名字，使得程序知道这个变量，可以通过extern获得一个不是定义的声明。使用其他文件的变量需要声明该变量。
  + 定义definition：一种声明，此外还分配内存、提供初值。只能一次。
  
+ 初始化：加`int a{10};`的初始化（list initialization）是更严格的初始化（如会导致丢失信息的初始化会报错）。
...
+ const修饰的全局变量作用域为单个文件内，想多个文件使用，则在定义和声明都使用extern修饰。constexpr常量表达式。constexpr int *q = nullptr，q指向整型的常量指针。
...
+ `decltype`获取变量类型。`decltype((i))`是一个引用类型。
...
相同类型数据的集合，类模板。`#include<vector> using std::vector;`

+ 初始化：`vector<int> vec,vec1(n,val),vec2(n,val=0),vec3{1,2},vec4(vec);`
+ 操作：`vec.push_back(val),`其他和string类似。
...
+ 起始位置`.begin()`、终止位置`.end()`。
+ 容器指针操作：比较`==,!=`，`++it,--it，it+=n`，`*it`取值；类型：常量指针`vector<int>::const_iterator it`、`vector<int>::const_iterator it`。

### 表达式

+ lvalue和rvalue：
+ 操作数Operands和操作符Operator：
+ Precedence优先级和Associativity结合性（同一优先级）：
+ static_cast：
...
+ assert and NDEBUG：
+ 函数匹配Function Matching：
+ 函数指针Pointers to Functions：

### 类

+ 访问控制和封装Encapsulation：
